...
class DefaultController extends YController {
	...
	*/
	...
	public function feed() {
		...
		$feed_items = YTable::getInstance('item')->getFromCategory($this->application->id, array_keys($categories), true);
	...
	protected function _getAlphaindex() {
		...
		if ($add_alpha_index == 2 || $add_alpha_index == 3) {
			...
			$alpha_index->addObjects(YTable::getInstance('item')->findAll(YTable::getInstance('item')->findAll($this->application->id, true, null, $options)), 'name');
		}
		return $alpha_index;
	}
