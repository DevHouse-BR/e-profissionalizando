...
else
{
	...
	if (isset($check4[1])) {
		...
		if($type == "premium")
		{
			downloadf($rslink,$fsize,$prem,$mirrors);
		}
		else
		{
			downloadfree($rslink,$fsize,$prem,$mirrors);
		}
		curl_close($curl);
		exit;
	}
    ...
    function downloadf($link,$filesize,$prem,$mirrors)
    {
    	ob_start();
    	// RS Link
        $url = @parse_url($link);
        ...
        function downloadfree($link,$filesize,$prem,$mirrors)
    {
    	ob_start();
    	// RS Link
        $url = @parse_url($link);
