...
$check4 = explode("collected",$xxx);
if (isset($check4[1])) {
	
$points = get_value_from_code('RapidPoints:</td><td style="padding-right:20px;"><b>','</b></td>', $xxx);
	//$mypoints = explode ("</b><a href=\"http://rapidshare.com/faq.html\" target=\"_blank\">Premium-Points</a>",$check4[1]);
	//$points = $mypoints[0];
//$points = substr($check4[1],0,5);
//preg_match( "#collected <b>(.+?)</b><a href=\"http://rapidshare.com/faq.html\" target=\"_blank\">Premium-Points</a>#", $check4[1], $points );
//die("expired");
//die($check1[1]);
}
//die("expired");
...
$today = date('dmy');
$bandw = 0;
$source = mysql_query("SELECT * FROM band WHERE disabled != 'expired' and disabled != 'bandwidth' and disabled != 'unknown' and disabled != 'incorrect'") 
or die(mysql_error()); 
$premnum = 0;
$totalbw = 0;
