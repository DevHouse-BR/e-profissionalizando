	...
	*/
	public static function installApplicationFromUserfile($userfile) {
		...
		if (is_writable($tmp_directory)) {
			...
			$tmp_directory = uniqid($tmp_directory.'install_').'/';
			JFolder::create($tmp_directory);

			$zip = new PclZip($userfile['tmp_name']);
			$zip->extract(PCLZIP_OPT_PATH, $tmp_directory);
			
			return self::installApplicationFromFolder($tmp_directory);		
			
		}
		
		throw new InstallHelperException('Directory is not writable. ('. $tmp_directory .')');
