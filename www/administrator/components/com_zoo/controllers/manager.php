...
class ManagerController extends YController {
			...
			try {
				...
				foreach (JFolder::folders($path, '.', false, true) as $template) {
					...
					if (!$renderer->pathExists('item'.DIRECTORY_SEPARATOR.$id)) {
						...
						$renderer->saveConfig($config, $template.'/renderer/item/positions.config');
					}
				}				
	...
	public function saveType() {
		...
		// filter identifier
		$post['identifier'] = JFilterOutput::stringURLSafe($post['identifier'] == '' ? $post['name'] : $post['identifier']);
