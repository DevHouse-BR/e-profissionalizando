...
class ItemController extends YController {
	...
	public function display() {
		...
		// get request vars
		$filter_item		= YRequest::getInt('item_filter', 0);
		$this->type_filter	= YRequest::getArray('type_filter', array());
		$state_prefix       = $this->option.'_'.$this->application->id.'.item.';
		...
		$this->lists['select_type'] = JHTML::_('zoo.typelist',  $options, 'filter_type', 'class="inputbox auto-submit"', 'value', 'text', $filter_type);
	...
	public function save() {
		...
		try {
			...
			// set alias
			$item->alias = ItemHelper::getUniqueAlias($item->id, JFilterOutput::stringURLSafe($item->alias));
